...
async function fetchMovies(page = 1) {
    const movies = await fetchContent('movie', page, currentGenre, currentSortOption,language);
    
    if (movies.results.length > 0) {
        totalPages = movies.total_pages; // ✅ Get total pages from API response
        currentPage = page; // ✅ Update current page
        displayContent(container, movies.results, 'movie');
...
searchButton.addEventListener('click', async () => {
    ...
    if (query) {
        const movies = await fetchSearchResults('movie', query);
        displayContent(container, movies, 'movie');
    } else {
...
async function setupGenres() {
    try {
        const genres = await fetchGenres('movie'); // Fetch genres from API
