...
let apiUrl = `https://api.themoviedb.org/3/movie/${id}?api_key=${apiKey}&language=${language}`;
let castUrl = `https://api.themoviedb.org/3/movie/${id}/credits?api_key=${apiKey}&language=${language}`;
...
listel.addEventListener("click", () => {
    ...
    if (index !== -1) {
        ...
        listStorage.push({ id: movieId, kind: "movie" });
...
favel.addEventListener("click", () => {
    ...
    if (index !== -1) {
        ...
        // Movie is NOT in watchlist → Add it
        favStorage.push({ id: movieId, kind: "movie" });
        favicon.classList.replace("fa-regular", "fa-solid");
